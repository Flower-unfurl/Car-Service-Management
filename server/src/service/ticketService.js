...
const ticketService = {
    ...
    createTicket: async (ticketData, actor) => {
        ...
        if (!ticketType) {
            throw new ErrorException(
                400,
                "Loại phiếu không hợp lệ. Chỉ chấp nhận PARKING hoặc SERVICE.",
            );
        }
    ...
    createOrUpdateServiceOrder: async (
        ticketId,
        serviceIds,
        session = null,
    ) => {
        ...
        if (ticket.ticketType !== SERVICE_TICKET_TYPE) {
            throw new ErrorException(
                400,
                "Ticket type must be SERVICE to create service order",
            );
        }
        ...
        if (missingServiceIds.length > 0) {
            throw new ErrorException(
                400,
                `Một số dịch vụ không hợp lệ: ${missingServiceIds.join(", ")}`,
            );
        }
        ...
        const latestOrder = await Order.findOne({ ticketId: ticket._id })
            .sort({ createdAt: -1 })
            .session(session);
        ...
        if (latestOrder && latestOrder.invoiceStatus === "CONFIRMED") {
            throw new ErrorException(
                409,
                "Invoice already confirmed. Cannot update service order.",
            );
        }
        ...
        if (latestOrder) {
            ...
            await latestOrder.save({ session });
            return latestOrder;
        }
    ...
    createFullEntry: async (fullData, actor) => {
        ...
        if (ticket.ticketType === SERVICE_TICKET_TYPE) {
            if (!inspectionData) {
                throw new ErrorException(
                    400,
                    "Inspection data is required for SERVICE tickets",
                );
            }
            ...
            if (normalizedServiceIds.length > 0) {
                createdOrder = await ticketService.createOrUpdateServiceOrder(
                    ticket._id,
        ...
        const resultTicket = await Ticket.findById(ticket._id).populate(
            "inspection",
        );
    ...
    addServicesToTicketFlow: async (ticketId, serviceIds, actor) => {
        ...
        try {
            ...
            if (ticket.ticketType !== SERVICE_TICKET_TYPE) {
                throw new ErrorException(
                    400,
                    "Only SERVICE ticket supports service flow",
                );
            }
            ...
            if (!ticket.inspection) {
                throw new ErrorException(
                    400,
                    "Inspection must be completed before selecting services",
                );
            }

            const order = await ticketService.createOrUpdateServiceOrder(
                ticket._id,
                serviceIds,
                session,
            );
            ...
            const tasks = await serviceTaskService.createTasksFromOrder(
                order._id,
                {
                    defaultAssignedStaffId: shouldAssignDefaultStaff(actor)
                        ? actor._id
